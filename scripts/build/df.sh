...
get_df_dir() {
	echo "$(get_lnp_dir)/df_${CT_DF_VERSION}_linux"
}
...
do_df_get() {
    if [ "${CT_DF_CUSTOM}" = "y" ]; then
        ...
        CT_GetFile "df_${CT_DF_VERSION}_linux" ".tar.bz2" \
...
do_df_extract() {
    ...
    if [ "${CT_DF_CUSTOM}" = "y"                    \
         -a -d "${CT_SRC_DIR}/df_${CT_DF_VERSION}_linux" ]; then
        return 0
    fi
	lnp_dir=${CT_SRC_DIR}/lnp-${CT_LNP_VERSION}/
	df_dir=df_${CT_DF_VERSION}_linux
	if [ -d "$lnp_dir" ]; then
